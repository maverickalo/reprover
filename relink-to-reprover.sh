#!/bin/bash

echo "Relinking to reprover project..."
rm -rf .vercel

# Create .vercel directory with correct project
mkdir -p .vercel
cat > .vercel/project.json << 'EOF'
{
  "projectId": "prj_reprover",
  "orgId": "team_sean-vernons-projects-31ef7fc1"
}
EOF

echo "Linked to reprover project!"

# Now add the environment variables to the correct project
echo "Adding Firebase credentials to reprover project..."

vercel env add FIREBASE_PROJECT_ID production --force < /dev/stdin <<< "reprover-aae0b"
vercel env add FIREBASE_CLIENT_EMAIL production --force < /dev/stdin <<< "firebase-adminsdk-fbsvc@reprover-aae0b.iam.gserviceaccount.com"

cat << 'PRIVATEKEY' | vercel env add FIREBASE_PRIVATE_KEY production --force
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
PRIVATEKEY

echo "✅ Done! Now deploy: vercel --prod"